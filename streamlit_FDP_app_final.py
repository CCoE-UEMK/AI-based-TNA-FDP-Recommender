...
@st.cache_resource
def load_model():
    with open("tna_model_with_avg_withoutD31.pkl", "rb") as f:
...
st.write(
    "Predicts FDP Need (%), Highlights top subdomains, and applies smart rules to suggest FDPs."
)
...
for k in fdp_topic_map.keys():
    if use_slider == "Slider":
        scores[k] = st.sidebar.slider(k, 1.0, 9.0, 1.0, step=0.01)
    else:
        scores[k] = st.sidebar.number_input(k, min_value=1.0, max_value=9.0, value=1.0, step=0.01)
...
if hasattr(classifier, "feature_importances_"):
    ...
    st.dataframe(importance_df[["Feature", "Importance (%)"]].style.format({"Importance (%)": "{:.2f}"}))

    # Also plot
    fig, ax = plt.subplots(figsize=(8,6))
    importance_df.set_index("Feature")["Importance (%)"].sort_values().plot(kind="barh", color="teal", ax=ax)
    ax.set_xlabel("Importance (%)")
    ax.set_ylabel("Feature")
    ax.set_title("Feature Importances Learned by Model")
    st.pyplot(fig)

    # Note
    st.info(
        "ℹ️ These feature importances show how much each domain contributed to predicting FDP need "
        "based on your dataset. They indicate **predictive power**, not direct causation."
    )
else:
